...
from database import criar_usuario, create_tables, globalizar_cursor_e_conexao, connect_database, \
...
def create_app():
    app = Flask(__name__)
    app.config.from_object('config.Config')     # Importa as configurações do arquivo config.py (createdb,
    # createtables, etc)
    ...
    from routes.tutor import tutores_bp
    app.register_blueprint(tutores_bp)

    from routes.animal import animais_bp

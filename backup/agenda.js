...
async function carregarEspecialidades() {
    try {
        ...
        selectEsp.innerHTML = '<option value="">Selecione uma especialidade</option>';
...
async function carregarVeterinarios() {
    ...
    if (!especialidade || !turno) {
        return;
    }

    try {
        const response = await fetch(`/api/veterinarios_disponiveis?especialidade_id=${especialidade}&turno=${turno}`);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("turno").addEventListener("change", carregarVeterinarios);
    document.getElementById("especialidade").addEventListener("change", carregarVeterinarios);
});
